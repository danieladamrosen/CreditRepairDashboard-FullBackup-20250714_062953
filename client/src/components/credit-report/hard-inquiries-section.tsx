...
export function HardInquiriesSection({
  ...
  // Older Inquiries Save → Hard Inquiries Choreography
  useEffect(() => {
    if (olderInquiriesSaved && !hasOlderChoreographed) {
    ...
    // Reset choreography flag when save state changes to false
    if (!olderInquiriesSaved) {
      setHasOlderChoreographed(false);
      setHardInquiriesGreenShown(false);
    }
  }, [olderInquiriesSaved, hasOlderChoreographed, setHardCollapsed]);
  ...
  return (
    <div className="mb-4" ref={hardInquiriesRef}>
      {hardCollapsed ? (
        <Card className={`${
          allInquiriesSaved || hardInquiriesGreenShown ? 'bg-green-50 border-[2px] border-green-500' : 'border border-gray-200'
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className={`text-lg font-bold ${allInquiriesSaved || hardInquiriesGreenShown ? 'text-green-700' : 'text-gray-900'}`}>
                ...
                <p className={`text-sm ${
                  ...
                  recentInquiriesCount > 0 ? 'text-orange-600' : 'text-gray-500'
                }`}>
                  {allInquiriesSaved || hardInquiriesGreenShown ? 
            ...
            <div className="flex items-center gap-1">
              <span className={`text-sm ${allInquiriesSaved || hardInquiriesGreenShown ? 'text-green-700' : 'text-gray-600'}`}>
                {totalInquiriesCount} inquiries
              </span>
              <ChevronDown className={allInquiriesSaved || hardInquiriesGreenShown ? 'text-green-700' : 'text-gray-600'} />
            </div>
          </CardHeader>
        </Card>
      ) : (
